...
import com.kanfa.news.admin.feign.IChannelContentServiceFeign;
...
public class LiveRest {
    ...
    @Autowired
    private IChannelContentServiceFeign ichannelContentServiceFeign;
    ...
    public  ObjectRestResponse onPublicLive(@PathVariable("id")Integer id){
        List<ChannelContent> liveChannelContents = liveServiceFeign.getLiveChannelContents(id);
        for (ChannelContent channelContent:liveChannelContents) {
            channelContent.setIsPublish(1);
            ichannelContentServiceFeign.update(channelContent.getId(),channelContent);
        }
        Live live = new Live();
        live.setId(id);
        live.setIsPublish(1);
