...
import com.kanfa.news.data.client.feign.IActivitySynDataServiceFeign;
 ...
 */
@RestController
@RequestMapping("/activitySynchrodataRest")
public class ActivitySynchrodataRest extends BaseRPC{

    @Autowired
    private IActivitySynDataServiceFeign activitySynDataServiceFeign;
    @Autowired
     ...
     * @return
     */
    @RequestMapping(value = "/addContentActivity", method = RequestMethod.GET)
    public ObjectRestResponse<ActivityInfo> addContentActivity(@RequestParam("id") Integer id) {
        if (null==id){
            String msg="id不能为空";
            return getObjectRestResponse(msg);
        }
        ActivityInfo entity=new ActivityInfo();
        ObjectRestResponse<com.kanfa.news.data.common.vo.content.ActivityInfo> contentInfo = activitySynDataServiceFeign.getContentActivity(id);
        com.kanfa.news.data.common.vo.content.ActivityInfo data = contentInfo.getData();
