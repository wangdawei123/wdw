...
var pinglun_m2 = function (page,type) {
    ...
    url=getPrefixUrl().toString().substr(0,url.length-1);
    $.ajax({
        url: url + 'comment/ajaxGetList',
        dataType:"json",
        type: 'POST',
        data: {
            cid: _CONST_.cid,
...
function getCommentHtml(data,allcomment) {
    ...
    for (var i = 0, len = data.commentList.length; i < len; i++){
        ...
        html += '</div>';
        if(item.comment!=undefined && item.comment.length){
