var OssUpload = {
    ...
    uploader: function () {
        ...
        this.upObj = new plupload.Uploader({
            ...
            filters: {
                mime_types : [ //只允许上传图片
                    { title : "Image files", extensions : "txt" },
                ],
    ...
    upload_progress: function(up, file) {
        $('#couponcode').parents('.input-group:first').find(":text").val("上传中……");
    },
    // 上传完成
    finished: function (up, file, info) {
        ...
        {
            $('#couponcode').parents('.input-group:first').find(":text").val(__this.filesUrl[file.name]);
            //$('#image_preview').show();
