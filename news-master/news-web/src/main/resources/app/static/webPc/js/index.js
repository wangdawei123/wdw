    ...
    $(window).scroll(function(){
        ...
        if($(document).scrollTop()>=$(document).height()-$(window).height()){
            ...
            $.ajax({
                ...
                dataType: 'json',
                url: '/contentLoad',
                data: {page:page,limit:limit,channelId:channelId},
                            ...
                            switch ( data[i].contentType){
                                ...
                                case 2:if($.trim(data[i].image)){
                                    ...
                                    resultHtml += '<li class="new-line big-imagetext-news">'+
                                        ...
                                        '</h3>'+
                                        '<p>'+toString(data[i].createTime)+'</p>'+
                                        '</div>' +
                                        '</li>';
                                }else{
                                ...
                                case 3:
                                   resultHtml += '<li class="new-line big-img-news">'+
                                    ' <a href="/detail/'+data[i].id+'/'+data[i].contentType+'" class="image-box">'+
                                    '<div><img src="'+data[i].imageList[0]+'" alt="'+data[i].title+'"></div>'+
                                    '<div><img  src="'+data[i].imageList[1]+'" alt="'+data[i].title+'"></div>'+
                                    '<div><img  src="'+(data.length > 2)?+'"/webPc/images/weixin.png"':data[i].imageList[2]+'" alt="'+data[i].title+'"></div>'+
                                    '<div><img  src="'+(data.length > 3)?+'/webPc/images/weixin.png':data[i].imageList[3]+'" alt="'+data[i].title+'"></div>'+
