...
public class CommentBiz extends BaseBiz<CommentMapper, Comment> {
    ...
    /**
     * 获取分享页评论列表 展示一级内容，即使回复的内容也按主体处理，不显示父级
     */
    public HashMap<String, Object> getWebCommentList(Integer liveId ,Integer type ,Integer page ,Integer count ,Integer uid){
        //1：先审后发，2：先发后审
        String opsSet = settingBiz.selectByname("comment_ops");
        if(LiveCommonEnum.CONTENT_LIVE.equals(type)){
            opsSet = LiveCommonEnum.OPS_TRUE+"";
        }
        List<CommentInfo> list = commentMapper.getListByConditions(liveId,Integer.parseInt(opsSet), type ,uid ,(page-1)*count ,count);
        if(list.size() == (LiveCommonEnum.CONSTANT_ZERO)){
            return new HashMap<>(1);
        }
        HashMap<String, Object> data = new HashMap<>(8);
        for (CommentInfo comment : list) {
            if(!LiveCommonEnum.CONSTANT_ZERO.equals(comment.getCreateUid())){
                AppUser appUser = userBiz.selectById(comment.getCreateUid());
                data.put("create_user" ,appUser.getId());
                data.put("user_image" ,"".equals(appUser.getImage())?LiveCommonEnum.WEB_IMAGE_URL:appUser.getImage());
            }
            data.put("content",comment.getContent());
            data.put("create_time",comment.getCreate_time());
        }
    ...
    public Integer add(Integer uid ,String nickName ,
        ...
        comm.setCreateIp(IPv4Util.ipToInt(request.getRemoteHost()));
        if(type.equals(LiveCommonEnum.LIVE_COMMENT)){
            ...
            if(live.getCommentType().equals(LiveCommonEnum.COMMENT_ANSWER)){
                comm.setOps(LiveCommonEnum.COMMENT_ANSWER);
            }
        }else{
        ...
        int i = commentMapper.insertSelectiveComment(comm);
        return comm.getId();
    }
    ...
    public List<Map<String, Object>> getListByCreator(Integer uid , Integer page ,Integer pcount){
        ...
        List<Map<String, Object>>  comList = buildList(comments , c.getOps() ,true ,uid);
        return comList;
    }
    ...
    public List<Map<String, Object>>  getListByReceiver(Integer uid , Integer page ,Integer pcount){
        ...
        List<Map<String, Object>> comList = buildList2(comments , c.getOps() ,true ,uid);
        return comList;
    }
        ...
        for (CommentInfo comment : comments) {
            ...
            if(!LiveCommonEnum.CONSTANT_ZERO.equals(comment.getPid())){
                ...
                if(comm != null){
                    ...
                    comment2Map.put("create_time",comm.getCreateTime());
                ...
                commentMap.put("create_time",comment.getCreateTime());
...
//评论审核规则:   0-先审后发, 1-先发后审
    ...
    public List<Comment> getCommentListByCid(Integer cid) {
        return commentMapper.getCommentListByCid(cid);
    }
     ...
     * @return
     */
    public List<CommentInfo> getCommentPageForDetail(Integer id, Integer type,Integer uid) {
            return null;
    }
     ...
     * @return
     */
    public Integer getCountForDetail(Integer cid,Integer type,Integer uid,Integer ops) {
        return commentMapper.getCountForDetail(cid,type,uid,ops);
    }
}