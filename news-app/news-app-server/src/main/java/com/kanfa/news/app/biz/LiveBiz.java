...
import com.kanfa.news.app.vo.admin.live.Utils.JsonChangeUtil;
...
public class LiveBiz extends BaseBiz<LiveMapper,Live> {
     ...
     * 2018-7-30
     */
    public List<Map<String  ,Object>> getNewLiveData(Integer page,Integer pcount,Integer signtime){
        List<LiveInfo> list = liveMapper.getNewLiveData(page, pcount, (page - 1) * pcount, signtime==0?null:DateUtil.dfs.format(signtime));
        ...
        for(LiveInfo liveInfo : list){
            ...
            map.put("duration",StringToDuration.changeToFormat(liveInfo.getLiveDuration()+""));
            map.put("card_type" ,LiveCommonEnum.CARD_TYPE_LIVE);
            map.put("type" ,LiveCommonEnum.CONTENT_LIVE);
            Map<String  ,Object> share = new HashMap<>(16);
            share.put("title" ,liveInfo.getTitle());
            if(liveInfo.getCoverImg() != null || liveInfo.getCoverImg() != ""){
                share.put("img" ,liveInfo.getCoverImg());
            }else {
                share.put("img" ,LiveCommonEnum.ACTIVITY_LAW_ICON);
            }
            share.put("url" ,LiveCommonEnum.URL_PREFIX + "public/live/index.html?id=" + liveInfo.getId() + "&share=1&locf=out");
            map.put("share" , share);
            map.put("id" , liveInfo.getId());
            map.put("title" , liveInfo.getTitle());
            map.put("stat" , liveInfo.getLiveStatus());
            map.put("start_time" , liveInfo.getStart_time());
            map.put("live_url" , liveInfo.getLiveUrl());
            map.put("preview_url" , liveInfo.getPreviewUrl());
            map.put("preview_img" , liveInfo.getPreviewImg());
            map.put("review_url" , liveInfo.getReviewUrl());
            map.put("cover_img" , liveInfo.getCoverImg());
            map.put("views" , liveInfo.getViewCount());
            map.put("is_show_view_user" , liveInfo.getIs_show_view_user());
            map.put("name" , liveInfo.getName());
            map.put("image" , liveInfo.getImage());
            data.add(map);
        }
        return data;
    }
    ...
    public List<Map<String,Object>> getFilterList(Integer liveTypId ,Integer stat ,Integer type ,Integer case_type ,
        ...
        if(list.size() > LiveCommonEnum.CONSTANT_ZERO && list != null){
            for(LiveInfo live : list){
                ...
                map.put("stat",live.getLiveStatus());
    ...
    public void updateFavs(Integer id , Integer type ){
            ...
            Live live = new Live();
            live.setId(id);
            Live newLive = liveMapper.selectByPrimaryKey(id);
    ...
    public Integer updateCommentCount(Integer id , Integer type){
        Live newLive = liveMapper.selectByPrimaryKey(id);
        if(newLive != null && type != null) {
            if (type.equals(LiveCommonEnum.CONSTANT_ONE)) {
                newLive.setCommentCount(newLive.getCommentCount() - LiveCommonEnum.CONSTANT_ONE);
            } else {
                newLive.setCommentCount(newLive.getCommentCount() + LiveCommonEnum.CONSTANT_ONE);
            }
        }
        int i = liveMapper.updateByPrimaryKeySelective(newLive);
        return i;
    }
    ...
    public List<Map<String,Object>> getBindLiveList(Integer id , @RequestParam(value = "0") Integer isapi){
        ...
        if(bindVideoList != null && bindVideoList.size() > 0){
            if(bindLiveList != null && bindLiveList.size() > 0){
                bindLiveList.addAll(bindVideoList);
            }else {
                bindLiveList = bindVideoList;
            }
        }
        for(LiveVideoBindInfo bindInfo :bindLiveList){
    ...
    public Map<String,Object> getLiveDetail( Integer liveId , Integer liveTypeId) {
        ...
        Live live = liveMapper.selectByPrimaryKey(liveId);
        info.setId(liveId);
        info.setCourtId(live.getCourtId());
        info.setLiveTypeId(liveTypeId);
        ...
        if(liveDetail != null){
            ...
            if(liveDetail.getLiveTypeId().equals(LiveCommonEnum.LIVE_TYPE_COURT)){
                ...
                if(liveDetail.getSourceId().equals(LiveCommonEnum.CONSTANT_ZERO)){
