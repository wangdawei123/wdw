...
//                    }
            ...
            if (null != content.get("pub_time")) {
                content.put("pub_time", DateUtil.fromTodayFormat((Date) content.get("pub_time")));
            }
...
//        List<Map> contentVideoDetailById = contentMapper.getContentVideoDetailById(data.getId());
     ...
     * @return
     */
    public ObjectRestResponse<Map<String, Object>> getVideoList(String devID,Integer chanId, Integer uid, Long signtime, Integer videoType,
                                                                String deviceImei, String channel, Integer page, Integer pcount) {
        ObjectRestResponse channelObjectRestResponse = new ObjectRestResponse();
        //返回结果集
        Map result = new HashMap(16);
        ...
        //查询频道的内容信息
        List<Map> newVideoContentByChannelId = channelContentMapper.getNewVideoContentListByChannelId(params);
        if(newVideoContentByChannelId==null || newVideoContentByChannelId.size()==0){
            result.put("list", null);
            channelObjectRestResponse.setData(result);
            return channelObjectRestResponse;
        }
        //查询频道信息
        Channel channelParm = new Channel();
        channelParm.setId(chanId);
        Channel channel1 = channelMapper.selectByPrimaryKey(channelParm);
        Integer hot_threshold = channel1.getHotThreshold();
        //将不同类型的资讯的内容补全
        newVideoContentByChannelId = contentMsgAdd(newVideoContentByChannelId, hot_threshold, dayNum);
        if(chanId==59 && page==1){
            Map vedioAlbumList = new HashMap(16);
            //类型，18为焦点图
            vedioAlbumList.put("card_type",18);
            // 获取所有上线专辑 limit 6
            List<Map> videoAlbum = videoAlbumMapper.getVideoAlbum();
            vedioAlbumList.put("content",videoAlbum);
            resultList.add(vedioAlbumList);
        }
        resultList.addAll(newVideoContentByChannelId);
