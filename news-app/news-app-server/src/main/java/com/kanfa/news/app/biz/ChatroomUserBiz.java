...
import com.kanfa.news.app.entity.ChatroomUser;
import com.kanfa.news.app.mapper.AppUserMapper;
import com.kanfa.news.app.mapper.ChatroomUserMapper;
...
public class ChatroomUserBiz extends BaseBiz<ChatroomUserMapper, ChatroomUser> {
    ...
    public Boolean refreshNickName(Integer uid , String name){
        ...
        c.setUid(uid+"");
        List<ChatroomUser> chatroomUsers = chatroomUserMapper.select(c);
        ChatroomUser chatroomUser = null;
        if(chatroomUsers != null && chatroomUsers.size() > 0){
            chatroomUser = chatroomUsers.get(0);
        }
        ...
        try{
            result = getRYSdk().user.refresh(uid + "", name, "http://kanfaimage.oss-cn-beijing.aliyuncs.com/defaultlogo.png");
        }catch (Exception e){
            e.printStackTrace();
        }
    ...
    public String createChatRoom(Integer contentId ){
        ...
        if(contentId != null) {
            ...
            try{
                CodeSuccessResult result= getRYSdk().chatroom.create(arr);
                if(result.getCode()==200){
                    return id;
                }
            }catch (Exception e){
    ...
    public ObjectRestResponse<String> getToken(Integer uid,
        ...
        if (chatRoomUserList != null && chatRoomUserList.size() > 0) {
            ...
            try {
                TokenResult tokenResult = getRYSdk().user.getToken(String.valueOf(uid), appUserInfo.getNickname(), image);
