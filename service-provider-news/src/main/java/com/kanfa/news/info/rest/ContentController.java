...
public class ContentController extends BaseController<ContentBiz, Content> {
    ...
    //个人浏览redis
    private final String IS_VIEW="CONTENTVIEW:is_view_";
    private final String WEB_VIEW="CONTENTVIEW:is_web_view_";
     ...
     * @return
     */
    @RequestMapping(value = "/content_Article/{id}/{clientIp}", method = RequestMethod.GET)
    public ContentInfo getContentArticle(@PathVariable(name = "id") int id,
                                         @PathVariable("clientIp") String clientIp) {
        Boolean flag = false;
        String key=WEB_VIEW+id+"_"+clientIp;
        if (!redisTemplate.hasKey(key)){
            //如果redis里没有存在这个ip,,说明是第一次访问
            redisTemplate.opsForValue().set(key,1,300, TimeUnit.SECONDS);
            flag = true;
        }
        return contentBiz.getContentArticle(id,flag);
    }
     ...
     * @return
     */
    @RequestMapping(value = "/getContentVideoById/{id}/{clientIp}", method = RequestMethod.GET)
    public ContentInfo getContentVideoById(@PathVariable int id,
        ...
        return contentBiz.getContentVideoById(id,flag);
    }
...
//                kpiCountContentInfo.setId(count.getId());
    ...
    public ObjectRestResponse updateContentView(@RequestBody ViewContent viewContent){
        ...
        if("0:0:0:0:0:0:0:1".equals(viewContent.getIp())){
            viewContent.setIp("127.0.0.1");
        }
