...
public class ContentBroadcastBindBiz extends BaseBiz<ContentBroadcastBindMapper,ContentBroadcastBind> {
     ...
     * @return
     */
    public void batchBindAndDelete(List<Integer> ids, Integer contentId) {
        if(contentId!=null){
            ContentBroadcastBind contentBroadcastBind= new ContentBroadcastBind();
            contentBroadcastBind.setContentId(contentId);
            contentBroadcastBindMapper.delete(contentBroadcastBind);
        }
        if(ids!=null && ids.size()>0){
            for (Integer bindId : ids) {
                ContentBroadcastBind contentBroadcastBind1 =new ContentBroadcastBind();
                contentBroadcastBind1.setContentId(contentId);
                contentBroadcastBind1.setBindId(bindId);
                contentBroadcastBindMapper.insertSelective(contentBroadcastBind1);
            }
        }
    }
}