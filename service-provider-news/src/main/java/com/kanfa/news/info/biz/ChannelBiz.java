...
import com.kanfa.news.info.mapper.ChannelCustomMapper;
import com.kanfa.news.info.mapper.ChannelFocusMapper;
import com.kanfa.news.info.mapper.ChannelMapper;
import com.kanfa.news.info.vo.admin.app.AppChannel;
...
public class ChannelBiz extends BaseBiz<ChannelMapper, Channel> {
    ...
    public List<ChannelInfo> selectChannlList(Channel entity  ,Integer channelId) {
        ...
        for(int i = 6 ; i < list.size() ; i++){
            if(channelId.equals(list.get(i).getId())){
                data.add(6,list.get(i));
                list.remove(i);
                break;
            }
        }
    ...
    public List<ChannelInfo> selectChannlList2(Channel entity) {
        return channelMapper.selectChannlList(entity);
    }
    ...
    public List<Map<String ,Object>> getKeywordData(Integer chanid ,Integer page , Integer pcount ,Integer cate ,String keyword ,Integer pub , Integer type){
        ...
        if(type.equals(LiveCommonEnum.SSPECIAL_TYPE_LIVE)){
            //当type ==2时
            List<LiveInfo> listType2 = channelMapper.getKeywordDataType2(offset ,pcount ,keyword);
    ...
    public VideoChannelAddInfo insertVideoChannel(Channel entity) {
        if (entity.getTopNum()==null){
            Integer integer = channelMapper.selectMaxOrderNumber(2);
            if (integer==null){
                Integer num = 1 ;
                entity.setTopNum(num);
            }else {
                entity.setTopNum(integer+1);
            }
        }
     ...
     * @return
     */
    public List<Channel> getChannelListForContentPage(ChannelInfo entity) {
       return channelMapper.getChannelListForContentPage(entity);
    }
     ...
     * @return
     */
    public ObjectRestResponse<Map<String, List<Channel>>> newVideoListChannels(){
        List<Channel> videoChannel = channelMapper.getVideoChannel();
        List<Channel> pcLeftChannel = channelMapper.getPCLeftChannel();
        String appChannels = "appChannels";
        String pcChannels = "pcChannels";
        Map<String, List<Channel>> stringListMap = new HashMap<>(16);
        stringListMap.put(appChannels,videoChannel);
        stringListMap.put(pcChannels,pcLeftChannel);
        ObjectRestResponse<Map<String, List<Channel>>> mapObjectRestResponse = new ObjectRestResponse<>();
        mapObjectRestResponse.setData(stringListMap);
        return mapObjectRestResponse;
    }


   public Integer selectMaxOrderNumber(Integer cate){
        return channelMapper.selectMaxOrderNumber(cate);
    }
}