...
public class LiveSpecialBindController extends BaseController<LiveSpecialBindBiz,LiveSpecialBind> {
   ...
   public Integer getMaxSort(@RequestParam("liveSpecialId") Integer liveSpecialId){
    ...
    //直播专题的通过liveSpecialId查询该集合
    @RequestMapping(value = "/getAllBinds",method = RequestMethod.GET)
    @ResponseBody
    public List<LiveSpecialBind> getAllBinds(@RequestParam("liveSpecialId")Integer liveSpecialId){
        LiveSpecialBind liveSpecialBind = new LiveSpecialBind();
        liveSpecialBind.setLiveSpecialId(liveSpecialId);
        return liveSpecialBindBiz.selectByExample(liveSpecialBind);
    }
